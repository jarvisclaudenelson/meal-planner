...
export default function RecipePicker({ open, onClose, onSelect, recipes = [], filter }) {
  ...
  return (
    <>
      ...
      {/* Sheet */}
      <div className="fixed bottom-0 left-0 right-0 z-50 bg-gray-800 rounded-t-2xl shadow-2xl animate-slide-up flex flex-col max-h-[85vh]">
        {/* Handle bar */}
        ...
        <div className="flex items-center justify-between px-4 pb-3 border-b border-gray-700">
          <h2 className="text-lg font-semibold text-gray-100">Pick a Recipe</h2>
        ...
        <div className="px-4 py-3">
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-9 pr-4 py-2 bg-gray-700 rounded-lg text-sm text-gray-100 outline-none focus:ring-2 focus:ring-emerald-400 placeholder-gray-500"
            />
          </div>
        </div>
        ...
        <div className="overflow-y-auto flex-1 px-4 pb-6 space-y-2">
          {filtered.length === 0 && (
            <p className="text-center text-gray-500 text-sm py-8">No recipes found</p>
          )}
          {filtered.map((recipe) => (
            <button
              key={recipe.id}
              onClick={() => { onSelect(recipe); onClose() }}
              className="w-full text-left p-3 rounded-xl border border-gray-700 hover:border-emerald-500 hover:bg-emerald-900/30 transition-colors"
            >
              <div className="flex items-start gap-2">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-1.5">
                    <span className="font-medium text-gray-100 text-sm truncate">{recipe.name}</span>
